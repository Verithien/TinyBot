module ArmoryBot
  module Commands
    module Pharah
      ...
      command([:pharah, :Pharah, :PHARAH], bucket: :overwatch, min_args: 3, rate_limit_message: 'All heroes share a rate limit. Wait %time% more seconds.') do |event, *account, region, platform|
        ...
        data = HTTParty.get("https://api.lootbox.eu/#{platform}/#{region}/#{acc}/hero/Pharah/", :verify => false ).parsed_response
        ...
        if data["statusCode"] == 500
          ...
          event.respond """#{event.user.mention} - #{name.capitalize} - Pharah
```ruby
- Hero Specific -
